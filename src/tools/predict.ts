import { execFile } from "child_process";
import path from "path";
import { Tool } from "./index";
import { PredictHeartDiseaseArgs } from "../types/predict";

type ExecFileResult = {
  stdout: string;
  stderr: string;
};

function runExecFile(
  command: string,
  args: string[],
  cwd: string,
): Promise<ExecFileResult> {
  return new Promise((resolve, reject) => {
    execFile(command, args, { cwd }, (error, stdout, stderr) => {
      if (error) {
        const commandError = new Error(stderr || error.message, {
          cause: error,
        });
        reject(commandError);
        return;
      }

      resolve({ stdout, stderr });
    });
  });
}

async function ensurePythonWithDeps(modelDir: string): Promise<string> {
  const requirementsPath = path.join(modelDir, "requirements.txt");
  const envPython = process.env.PYTHON_BIN;
  const localVenvPython = path.join(modelDir, ".venv", "bin", "python");

  const candidatePythons = [
    envPython,
    "/opt/venv/bin/python",
    localVenvPython,
    "python3",
    "python",
  ].filter((value): value is string => Boolean(value));

  // Track the first Python binary that exists (even without deps)
  // so we can use it to create a venv if needed
  let systemPython: string | null = null;

  for (const candidate of candidatePythons) {
    try {
      await runExecFile(
        candidate,
        ["-c", "import pandas, joblib, sklearn"],
        modelDir,
      );
      // All deps found — use this candidate directly
      return candidate;
    } catch {
      // Deps missing, but check if the binary itself exists
      if (systemPython === null) {
        await runExecFile(candidate, ["--version"], modelDir);
        systemPython = candidate;
      }
    }
  }

  // No candidate had all deps — try creating a local venv using
  // the first working Python binary we found
  if (systemPython === null) {
    throw new Error(
      "No Python binary found. Install Python 3 and ensure it is on PATH.",
    );
  }

  try {
    await runExecFile(
      systemPython,
      ["-m", "venv", path.join(modelDir, ".venv")],
      modelDir,
    );
    await runExecFile(
      localVenvPython,
      ["-m", "pip", "install", "--no-cache-dir", "-r", requirementsPath],
      modelDir,
    );
    await runExecFile(
      localVenvPython,
      ["-c", "import pandas, joblib, sklearn"],
      modelDir,
    );
    return localVenvPython;
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Python dependencies are missing and auto-install failed. Reason: ${reason}`,
      { cause: error },
    );
  }
}

export const predictHeartDiseaseTool: Tool<PredictHeartDiseaseArgs> = {
  definition: {
    name: "predict_heart_disease",
    description:
      "Predict the risk of heart disease based on patient clinical data.",
    inputSchema: {
      type: "object",
      properties: {
        age: { type: "number", description: "Age of the patient" },
        sex: { type: "number", description: "Sex (1: Male, 0: Female)" },
        resting_bp: {
          type: "number",
          description: "Resting blood pressure (mm Hg)",
        },
        cholesterol: {
          type: "number",
          description: "Serum cholesterol (mm/dl)",
        },
        fasting_bs: {
          type: "number",
          description: "Fasting blood sugar > 120 mg/dl (1: True, 0: False)",
        },
        max_hr: { type: "number", description: "Maximum heart rate achieved" },
        exercise_angina: {
          type: "number",
          description: "Exercise induced angina (1: Yes, 0: No)",
        },
        oldpeak: {
          type: "number",
          description: "ST depression induced by exercise relative to rest",
        },
        chest_pain_type: {
          type: "string",
          enum: ["ASY", "NAP", "ATA", "TA"],
          description:
            "Chest pain type (ASY: Asymptomatic, NAP: Non-Anginal Pain, ATA: Atypical Angina, TA: Typical Angina)",
        },
        resting_ecg: {
          type: "string",
          enum: ["Normal", "ST", "LVH"],
          description: "Resting electrocardiogram results",
        },
        st_slope: {
          type: "string",
          enum: ["Flat", "Up", "Down"],
          description: "Slope of the peak exercise ST segment",
        },
      },
      required: [
        "age",
        "sex",
        "resting_bp",
        "cholesterol",
        "fasting_bs",
        "max_hr",
        "exercise_angina",
        "oldpeak",
        "chest_pain_type",
        "resting_ecg",
        "st_slope",
      ],
    },
  },
  handler: async (args: PredictHeartDiseaseArgs) => {
    const modelDir = path.join(process.cwd(), "ai_models", "heart_model");
    const scriptPath = path.join(modelDir, "predict_pipeline.py");
    const predictionArgs = [
      scriptPath,
      "--age",
      String(args.age),
      "--sex",
      String(args.sex),
      "--resting_bp",
      String(args.resting_bp),
      "--cholesterol",
      String(args.cholesterol),
      "--fasting_bs",
      String(args.fasting_bs),
      "--max_hr",
      String(args.max_hr),
      "--exercise_angina",
      String(args.exercise_angina),
      "--oldpeak",
      String(args.oldpeak),
      "--chest_pain_type",
      args.chest_pain_type,
      "--resting_ecg",
      args.resting_ecg,
      "--st_slope",
      args.st_slope,
    ];

    return new Promise((resolve) => {
      ensurePythonWithDeps(modelDir)
        .then((pythonPath) => runExecFile(pythonPath, predictionArgs, modelDir))
        .then(({ stdout }) => {
          try {
            const result = JSON.parse(stdout.trim());
            return resolve({
              content: [
                {
                  type: "text",
                  text: `Prediction Result:\n${result.result_text}\nProbability: ${(result.probability_of_disease * 100).toFixed(2)}%`,
                },
                {
                  type: "text",
                  text: JSON.stringify(result, null, 2),
                },
              ],
            });
          } catch {
            return resolve({
              content: [
                {
                  type: "text",
                  text: `Error parsing model output: ${stdout}`,
                },
              ],
              isError: true,
            });
          }
        })
        .catch((error) => {
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          return resolve({
            content: [
              {
                type: "text",
                text: `Error executing prediction: ${errorMessage}`,
              },
            ],
            isError: true,
          });
        });
    });
  },
};
